...
final class CommentController extends AbstractSocialController
{
    ...
    public function create(
        ...
        if (! $this->isCsrfTokenValid('comment_post_' . $post->getId(), (string) $request->request->get('_token'))) {
            $this->addFlash('error', 'Invalid comment request.');
        ...
        if ($body === '') {
            $this->addFlash('error', 'Comment text cannot be empty.');
        ...
        if ($parentCommentId > 0) {
            ...
            if (!$parentComment instanceof Comment || $parentComment->getPost()?->getId() !== $post->getId()) {
                $this->addFlash('error', 'The reply target could not be found.');
    ...
    public function delete(
        ...
        if ($post === null) {
            throw $this->createNotFoundException('Comment does not belong to a post.');
        }
