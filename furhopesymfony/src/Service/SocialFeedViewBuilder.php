...
final class SocialFeedViewBuilder
{
    ...
    public function buildFeed(User $currentUser, string $searchTerm): array
    {
        ...
        return array_merge($connections, [
            ...
            'stats' => [
                ['label' => 'Visible posts', 'value' => count($feedPosts)],
                ['label' => 'Friends', 'value' => count($friendIds)],
    ...
    private function buildUserSummary(?User $user): array
    {
        ...
        return [
            'entity' => $user,
            'id' => (int) $user?->getId(),
            'name' => $user?->getName() ?: 'Unknown member',
            'email' => $email,
    ...
    private function buildStoryCards(array $feedPosts): array
    {
        ...
        foreach ($feedPosts as $feedPost) {
            ...
            $storyCards[] = [
                ...
                'caption' => $this->excerpt((string) ($feedPost['caption'] ?: 'Fresh shelter update.')),
    ...
    private function notificationMeta(string $type): array
    {
        return match ($type) {
            'POST_LIKE' => [
                ...
                'message' => 'liked your animal update.',
                'cta' => 'Open post',
            ],
            'POST_DISLIKE' => [
                'icon' => 'feedback',
                'tone' => 'social-notification--feedback',
                'message' => 'reacted negatively to your post.',
                'cta' => 'Review post',
            ],
            'POST_COMMENT' => [
                'icon' => 'comment',
    ...
    {
        return $dateTime?->format('d M Y \a\t H:i') ?? 'Just now';
    }
