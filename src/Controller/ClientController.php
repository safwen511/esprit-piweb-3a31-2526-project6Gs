...
class ClientController extends AbstractController
{
    ...
    public function listVets(
        ...
        $client = $this->getUser();
        $vets = $em->getRepository(User::class)->findBy(['role' => 'VETERINAIRE']);
    ...
    public function submitReview(
        ...
        if (!$vetId || $note < 1 || $note > 5) {
            return $this->json(['success' => false, 'message' => 'Donnees invalides.']);
        }
        ...
        if (!$rdvConfirme) {
            return $this->json(['success' => false, 'message' => 'Aucun RDV confirme avec ce veterinaire.']);
        }

        $dejaAvis = $reviewRepo->findOneBy(['client' => $client, 'vet' => $vet]);
        if ($dejaAvis) {
            return $this->json(['success' => false, 'message' => 'Vous avez deja laisse un avis.']);
        }
    ...
    public function prendreRdv(int $id, Request $request, EntityManagerInterface $em, MailService $mailService): Response
    {
        ...
        $animals = $em->getRepository(Animal::class)->findBy(['ownerCompte' => $client]);
        ...
        if ($request->isMethod('POST')) {
            ...
            if (!$dispo || !$dispo->isAvailable()) {
                $this->addFlash('danger', 'Ce creneau n est plus disponible.');
            ...
            if ($newAnimalName && $newAnimalSpecies) {
                ...
                $animal->setSpecies($newAnimalSpecies);
                $animal->setOwnerCompte($client);
                $em->persist($animal);
            } else {
            ...
            if (!$animal) {
                $this->addFlash('danger', 'Veuillez selectionner ou ajouter un animal.');
            ...
            $this->addFlash('success', 'Rendez-vous demande. Le veterinaire va confirmer votre RDV.');
    ...
    public function annulerRdv(int $id, EntityManagerInterface $em): Response
    {
        ...
        $em->flush();
        $this->addFlash('info', 'Rendez-vous annule.');
