...
final class AnimalShopController extends AbstractController
{
    ...
    public function delete(Animal $animal, Request $request, EntityManagerInterface $entityManager): RedirectResponse|JsonResponse
    {
        $user = $this->getCurrentUser();
        ...
        if ($this->isCsrfTokenValid('delete-animal-'.$animal->getId(), (string) $request->request->get('_token'))) {
            ...
            if ($request->isXmlHttpRequest()) {
                return $this->json([
                    'success' => true,
                    'message' => 'pet_home.flash.animal_deleted',
                ]);
            }
        }
