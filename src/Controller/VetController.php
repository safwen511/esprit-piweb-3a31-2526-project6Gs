...
class VetController extends AbstractController
{
    ...
    public function acceptRdv(int $id, EntityManagerInterface $em, MailService $mailService): Response
    {
        ...
        if ($rdv->getStatus() !== 'pending') {
            $this->addFlash('danger', 'Seules les demandes en attente peuvent etre acceptees.');
        ...
        $this->addFlash(
            'success',
            $mailSent
                ? 'Rendez-vous accepte et client notifie par email.'
                : 'Rendez-vous accepte. Le mail de confirmation n a pas pu etre envoye.'
        );
    ...
    public function refuseRdv(int $id, EntityManagerInterface $em): Response
    {
        ...
        if ($rdv->getStatus() !== 'pending') {
            $this->addFlash('danger', 'Seules les demandes en attente peuvent etre refusees.');
    ...
    private function getVetUser(): User
    {
        ...
        if (!$user instanceof User || $user->getRole() !== 'VETERINAIRE') {
