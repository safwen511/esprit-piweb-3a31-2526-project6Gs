...
public class AccueilController {
    ...
    private static final String LUNA_IMAGE_URL =
            "https://images.unsplash.com/photo-1511044568932-338cba0ad803?auto=format&fit=crop&w=800&q=80";
    private static final String BENTO_IMAGE_URL =
            "https://images.unsplash.com/photo-1552053831-71594a27632d?auto=format&fit=crop&w=800&q=80";
    private static final String MILO_IMAGE_URL =
    ...
    @FXML
    private ImageView friendLunaImageView;

    @FXML
    private ImageView friendBentoImageView;

    @FXML
    private ImageView friendMiloImageView;
    ...
    private void initialize() {
        ...
        Platform.runLater(this::loadShowcaseImages);
    ...
    private Image loadAvatarImage(String imagePath) {
        ...
        try {
            if (trimmedPath.startsWith("http://")
                ...
                return new Image(trimmedPath, 96, 96, false, true, true);
            }
            java.io.File file = resolveProfileImageFile(trimmedPath);
            if (!file.exists()) {
                return null;
            }
            return new Image(file.toURI().toString(), 96, 96, false, true, true);
        } catch (Exception ignored) {
            return null;
        }
    }

    private java.io.File resolveProfileImageFile(String imagePath) {
        java.io.File directFile = new java.io.File(imagePath);
    ...
    private void loadShowcaseImages() {
        setBackgroundImage(heroImageView, HERO_IMAGE_URL, 460, 320);
        setBackgroundImage(friendLunaImageView, LUNA_IMAGE_URL, 235, 150);
        setBackgroundImage(friendBentoImageView, BENTO_IMAGE_URL, 235, 150);
        setBackgroundImage(friendMiloImageView, MILO_IMAGE_URL, 235, 150);
    }

    private void setBackgroundImage(ImageView imageView, String imageUrl, double width, double height) {
        if (imageView == null) {
            return;
        }
        imageView.setSmooth(true);
        imageView.setImage(new Image(imageUrl, width, height, false, true, true));
    }
