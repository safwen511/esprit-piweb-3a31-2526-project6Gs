...
public class ServiceReview {
    ...
    public void add(Review review) throws SQLException {
        String sql = "INSERT INTO review (client_id, vet_id, rdv_id, note, rating, commentaire, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())";
        try (PreparedStatement ps = database.getConnection().prepareStatement(sql)) {
            ...
            ps.setString(6, review.getCommentaire());
            ps.executeUpdate();
        }
    }

    public double getAverageRating(int vetId) throws SQLException {
        String sql = "SELECT AVG(COALESCE(note, rating)) FROM review WHERE vet_id = ?";
        try (PreparedStatement ps = database.getConnection().prepareStatement(sql)) {
            ps.setInt(1, vetId);
