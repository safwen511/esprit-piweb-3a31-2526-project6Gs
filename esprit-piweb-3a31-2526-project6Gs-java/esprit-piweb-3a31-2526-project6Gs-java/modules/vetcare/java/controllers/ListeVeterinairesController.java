...
public class ListeVeterinairesController {
    ...
    private void loadVeterinaires() {
        try {
            ...
            PreparedStatement ps = conn.prepareStatement(
                    "SELECT id, first_name, last_name, email, COALESCE(phone, phone_number) AS phone, NULL AS city " +
                            "FROM user " +
                            "WHERE COALESCE(is_active, 1) = 1 " +
                            "AND COALESCE(is_veteran_approved, 0) = 1 " +
                            "AND (COALESCE(roles, '') LIKE '%ROLE_VETERINAIRE%' OR UPPER(COALESCE(role, '')) = 'VETERINAIRE')"
            );
    ...
    private int getNombreRdv(int vetId) {
        try {
            Connection conn = MyDatabase.getInstance().getConnection();
            PreparedStatement ps = conn.prepareStatement(
                    "SELECT COUNT(*) FROM rendezvous WHERE vet_id = ? AND LOWER(status) NOT IN ('cancelled', 'annule')"
            );
            ps.setInt(1, vetId);
            ResultSet rs = ps.executeQuery();
