...
public class RendezvousListController {
    ...
    public void initialize() {
        filterBox.setItems(FXCollections.observableArrayList(
                "Tous", "EN_ATTENTE", "CONFIRME", "ANNULE"
        ));
    ...
    private VBox buildCard(Rendezvous rdv) {
        ...
        if (isFinishedActionEnabled() && "CONFIRME".equals(rdv.getStatus())) {
            HBox btnRow = new HBox(10);
            btnRow.setAlignment(javafx.geometry.Pos.CENTER_RIGHT);
    ...
    private void updateStatus(Rendezvous rdv, String newStatus) {
        try {
            ...
            if ("CONFIRME".equals(newStatus)) {
                serviceDisponibilite.updateStatut(rdv.getDisponibilite_id(), "false");
            } else if ("ANNULE".equals(newStatus)) {
                serviceDisponibilite.updateStatut(rdv.getDisponibilite_id(), "true");
            }
    ...
    private boolean isFinishedActionEnabled() {
        return false;
    }
